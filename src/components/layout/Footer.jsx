      ...
      <div className="container-custom py-12 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10">
        ...
        <div>
          ...
          <div className="flex gap-3 mt-5">
            {[Facebook, Twitter, Instagram, Youtube].map((Icon, i) => (<a key={i} href="#" className="w-9 h-9 rounded-full bg-surface-dark-foreground/10 flex items-center justify-center hover:bg-primary hover:text-primary-foreground transition-all">
                <Icon size={16}/>
              </a>))}
          </div>
        </div>
        ...
        <div>
          ...
          <ul className="space-y-2">
            {footerLinks.navigate.map((l) => (<li key={l.label}>
        ...
        <div>
          ...
          <ul className="space-y-2">
            {footerLinks.categories.map((c) => (<li key={c}>
        ...
        <div>
          ...
          <ul className="space-y-2">
            {footerLinks.brands.map((b) => (<li key={b}>
      ...
      <div className="border-t border-border/20">
        <div className="container-custom py-6 flex flex-col md:flex-row items-center justify-between gap-4 text-xs text-surface-dark-foreground/50">
          ...
          <div className="flex gap-4">
            {["Visa", "Mastercard", "PayPal", "Apple Pay"].map((p) => (<span key={p} className="px-3 py-1 rounded bg-surface-dark-foreground/10 text-surface-dark-foreground/60 text-xs font-medium">{p}</span>))}
          </div>
        </div>
      </div>
    </footer>);
