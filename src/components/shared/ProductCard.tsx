      ...
      <Link to={`/product/${product.id}`} className="group block glass-card-hover rounded-xl overflow-hidden">
        <div className="relative overflow-hidden bg-secondary/30 aspect-square">
          <img
            src={product.image}
            alt={product.name}
            className="w-full h-full object-contain p-4 image-zoom"
          />
          {product.badge && (
          ...
          <button
            className="absolute top-3 right-3 w-8 h-8 rounded-full bg-background/80 backdrop-blur-sm flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all hover:bg-primary hover:text-primary-foreground"
          >
            <Heart size={14} />
          </button>
          {!product.inStock && (
        ...
        <div className="p-4">
          ...
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              className="w-9 h-9 rounded-lg gradient-primary text-primary-foreground flex items-center justify-center hover:shadow-[var(--shadow-glow)] transition-all disabled:opacity-40"
            >
