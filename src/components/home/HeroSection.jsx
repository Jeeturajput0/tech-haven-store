...
export default function HeroSection() {
    return (<section className="relative flex min-h-[460px] items-center overflow-hidden bg-surface-dark md:min-h-[600px]">
      <img src={heroBanner} alt="Hero Banner" className="absolute inset-0 h-full w-full object-cover opacity-60"/>
      ...
      <div className="container-custom relative z-10 py-16 sm:py-20">
        <motion.div initial={{ opacity: 0, x: -40 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.7 }} className="max-w-xl">
          ...
          <Link to="/shop" className="inline-flex items-center gap-2 rounded-lg gradient-primary px-6 py-3 text-xs font-bold uppercase tracking-wider text-primary-foreground transition-all hover:-translate-y-0.5 hover:shadow-[var(--shadow-glow)] sm:px-8 sm:py-4 sm:text-sm">
            Shop Now
          </Link>
        </motion.div>
      </div>
