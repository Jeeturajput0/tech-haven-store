    ...
    <AdminDashboardProvider>
      <div className="min-h-screen bg-[radial-gradient(circle_at_top,_hsl(var(--primary)/0.18),_transparent_32%),linear-gradient(180deg,hsl(var(--background)),hsl(var(--secondary)))]">
        <div className="flex min-h-screen">
          ...
          <AnimatePresence>
            {mobileOpen ? (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-40 bg-slate-950/45 lg:hidden">
                <motion.div initial={{ x: -40 }} animate={{ x: 0 }} exit={{ x: -40 }} className="h-full w-72 overflow-y-auto bg-white px-4 py-5 text-slate-700">
                  ...
                  <div className="space-y-1.5">
                      ...
                      return (
                        <NavLink
                          ...
                          className={cn(
                            ...
                            isActive ? "bg-[#f8f05b] text-white" : "text-slate-600 hover:bg-slate-100 hover:text-slate-900",
                          )}
                        >
                    ...
                    <div className="pt-1">
                      <button
                        type="button"
                        onClick={() => setMobileMasterDataOpen((current) => !current)}
                        className="flex w-full items-center gap-3 rounded-2xl px-4 py-3 text-sm font-medium text-slate-600 transition-colors hover:bg-slate-100 hover:text-slate-900"
                      >
                        <Boxes size={19} strokeWidth={2} />
                        <span className="flex-1 text-left">Master data</span>
                        <ChevronDown
                          size={18}
                          className={cn("transition-transform duration-200", mobileMasterDataOpen && "rotate-180")}
                        />
                      </button>
