...
export function AdminSidebar({
  ...
  return (
    <motion.aside
      ...
      className="fixed left-0 top-0 z-40 hidden h-screen overflow-hidden border-r border-slate-200 bg-white text-slate-700 lg:block"
    >
      <div className="flex h-full flex-col overflow-y-auto px-3 py-5">
        <div className={cn("flex items-center gap-3", collapsed ? "justify-center" : "justify-between")}>
          {collapsed ? (
            <div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-[#f8695b] text-white shadow-lg shadow-[#f8695b]/25">
              <Store size={20} />
            </div>
          ) : (
            <div>
        ...
        {collapsed ? (
          <button
            type="button"
            onClick={onToggle}
            className="mt-6 flex h-10 w-10 items-center justify-center self-center rounded-xl text-slate-500 transition-colors hover:bg-slate-100 hover:text-slate-900"
          >
        ...
        <div className="mt-8 space-y-1.5 pb-6">
            ...
            return (
              <NavLink
                ...
                className={cn(
                  ...
                  isActive ? "bg-[#f8695b] text-white shadow-sm shadow-[#f8695b]/30" : "text-slate-600 hover:bg-slate-100 hover:text-slate-900",
                )}
              >
          ...
          <div className="pt-1">
            <button
              ...
              className={cn(
                "flex w-full items-center gap-3 rounded-2xl px-4 py-3 text-sm font-medium text-slate-600 transition-colors hover:bg-slate-100 hover:text-slate-900",
                collapsed && "justify-center px-2",
              )}
            >
              ...
              {!collapsed ? (
                <>
                  ...
                  <ChevronDown
                    size={18}
                    className={cn("transition-transform duration-200", masterDataOpen && "rotate-180")}
                  />
                </>
              ) : null}
            </button>
                ...
                {masterDataItems.map((item) => {
                    ...
                    return (
                      <NavLink
                        ...
                        className={cn(
                          ...
                          itemActive ? "bg-[#fff1ef] font-medium text-[#f8695b]" : "text-slate-500 hover:bg-slate-100 hover:text-slate-900",
                        )}
                      >
                  ...
                  return (
                    <button
                      key={item.label}
                      type="button"
                      className="flex w-full items-center gap-3 rounded-2xl px-4 py-2.5 text-sm text-slate-400 transition-colors hover:bg-slate-100 hover:text-slate-700"
                    >
                      <ItemIcon size={17} strokeWidth={2} />
                      <span>{item.label}</span>
                    </button>
