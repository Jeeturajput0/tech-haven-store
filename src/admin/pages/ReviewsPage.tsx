    ...
    <div className="space-y-8">
      ...
      <div className="grid gap-6 lg:grid-cols-2 xl:grid-cols-3">
        {reviews.map((review, index) => (
          <motion.div key={review.id} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.08 }}>
            <AdminCard className="h-full p-6">
              ...
              <div className="mt-4 flex gap-1">
                {Array.from({ length: 5 }).map((_, starIndex) => (
                  <motion.div key={starIndex} whileHover={{ scale: 1.15 }} className="text-primary drop-shadow-[0_0_8px_hsl(var(--primary)/0.45)]">
